...
**Estado:** READY FOR INDEPENDENT RE-REVIEW  
...
26. Como usuario, quiero continuar mi Recorrido principal de estudio desde otro dispositivo de forma secuencial, para poder alternar entre móvil y PC sin exigir edición concurrente del mismo intento.
...
33. Como usuario, quiero que una pregunta en blanco en examen entre en Falladas pendientes y reinicie a cero la racha de dominio, para no esconder lagunas simplemente por no contestar.
...
65. Como usuario, quiero que completar al menos una vez un examen oficial en un recorrido completo de estudio sin preguntas pendientes, o en Modo examen, deje el hito Finalizado, para saber qué exámenes ya he completado íntegramente.
...
71. Como usuario, quiero que un intento histórico nunca cambie porque después se corrija el Banco de exámenes, para conservar la fidelidad de lo que realmente hice; el catálogo actual muestra solo la última versión publicada y la revisión histórica carga la versión exacta usada por aquel intento.
72. Como usuario, quiero generar un Examen artificial de 75 preguntas de todo el banco, para practicar una mezcla nueva cada vez; si no existen al menos 75 Preguntas activas distintas, la generación debe bloquearse en vez de repetir o truncar.
...
- Cada examen publicado tendrá una identidad estable y versiones inmutables; los intentos históricos conservarán la referencia exacta a la versión utilizada.
- El catálogo principal mostrará únicamente la última versión publicada de cada examen. Las versiones anteriores seguirán disponibles como artefactos estáticos para poder revisar intentos históricos, pero no aparecerán como tarjetas independientes en el catálogo.
- La identidad estable de una pregunta se construirá a partir del examen de origen y su numeración fuente. `question_progress` se ancla a esa identidad y no a la versión del JSON. Una nueva versión no reinicia por sí sola falladas, racha o dominio; si una pregunta pasa a anulada deja de estar activa pero su progreso histórico se conserva.
...
- Una importación estructuralmente ambigua queda bloqueada hasta revisión humana. Son ejemplos bloqueantes: ausencia de respuesta oficial, número de opciones incompatible, recuentos que no pueden reconciliarse con la fuente o tratamiento irresoluble de anuladas/reservas. Son ejemplos de incidencia revisable: fragmentos de maquetación extraños, saltos de línea sospechosos o texto residual que no altere por sí solo la estructura parseada.
- El flujo de publicación de un examen nuevo separará generación automática y aprobación humana. La aprobación humana se materializa mediante la revisión y merge de un Pull Request que contiene el JSON generado y su informe de QA; no habrá interfaz de aprobación dentro de la PWA.
...
- Un Recorrido principal de estudio se completa automáticamente cuando todas las preguntas incluidas han quedado confirmadas y no existe ninguna pendiente/saltada. Si el usuario sale antes, el intento permanece activo y reanudable. Cambiar de Orden normal a Orden aleatorio, o al contrario, marca el recorrido anterior como abandonado antes de crear el nuevo.
- El sistema de dominio se modela por usuario y pregunta de origen, no por intento. Dos aciertos consecutivos después de haber fallado convierten la pregunta en Dominada; un nuevo fallo reinicia la racha. Una blanca en Modo examen se considera fallo a efectos de dominio y también reinicia la racha.
- Mientras exista un Modo examen activo para un usuario, no se admitirán nuevas respuestas interactivas en estudio o Sesión de falladas hasta que ese examen se entregue o expire. Esto evita actualizaciones simultáneas de dominio sobre la misma pregunta durante una simulación activa.
...
- El Modo examen usa `started_at` y `deadline_at` absolutos generados con hora de servidor al iniciar el intento. El frontend conserva el `deadline_at` y un desfase de reloj de servidor suficiente para mostrar la cuenta atrás sin convertirla en un contador pausado.
- Al alcanzar el deadline, el frontend bloquea inmediatamente la edición y finaliza/corrige el intento con las últimas respuestas conocidas. Si el dispositivo está sin conexión, el bloqueo ocurre localmente y la finalización pendiente se sincroniza al reconectar.
- La base de datos debe impedir escrituras de respuestas de examen posteriores a `deadline_at` usando la hora del servidor. No se pretende ofrecer antitrampas frente a manipulación deliberada del dispositivo; eso permanece fuera de alcance.
...
- El pool de Examen artificial excluye explícitamente Preguntas anuladas y Preguntas de reserva no utilizadas. Si existen menos de 75 registros activos distintos, la generación queda bloqueada con un mensaje al usuario; nunca se repiten preguntas ni se genera un examen truncado.
- El Examen artificial en Modo estudio no tiene límite de tiempo y reutiliza exactamente las reglas generales de Tiempo activo de estudio.
...
- El Tiempo activo de estudio se acumulará mediante un heartbeat simple en frontend únicamente mientras la página esté visible y exista actividad reciente del usuario. Al pasar la pestaña a background o superar el umbral de inactividad, la acumulación se detiene hasta la siguiente interacción.
- El autoguardado será no bloqueante. La UI podrá avanzar sin esperar a la confirmación remota de cada cambio.
- La tolerancia a cortes breves de red conservará localmente un snapshot/estado pendiente del intento y lo reintentará al recuperar conectividad. No se construirá un sistema transaccional offline general ni una cola compleja de operaciones.
- La persistencia remota mantendrá una revisión/versión monotónica del estado suficiente para rechazar una escritura local obsoleta si otro dispositivo ya ha avanzado ese mismo intento.
- El cambio entre móvil y PC está soportado de forma secuencial una vez sincronizado el estado. La edición simultánea del mismo intento desde dos dispositivos no es un caso soportado en la v1.
- La aplicación tendrá un módulo de persistencia que abstraiga sincronización remota y estado pendiente local de la lógica de estudio/examen.
...
- El Dashboard compartido se apoyará en una RPC PostgreSQL de agregados con privilegios controlados (`SECURITY DEFINER` o mecanismo equivalente endurecido) y esquema de salida fijo. Esa interfaz podrá leer lo necesario para calcular agregados de los tres usuarios, pero nunca devolverá filas de respuestas, intentos detallados ni otros datos privados crudos de otro usuario.
- El modelo dinámico mínimo se estructura alrededor de cuatro conceptos persistentes: perfil, intento, respuesta de intento y progreso por pregunta. No existe una entidad separada de `progreso principal de estudio`: el propio intento activo es la fuente de verdad del progreso reanudable.
- Un intento guarda como mínimo usuario, tipo de origen, examen/version cuando aplique, modo, estrategia, estado, cola/orden de preguntas, posición, tiempos, revisión monotónica de persistencia, agregados de resultado y score cuando aplique.
- Para un examen oficial puede existir como máximo un intento activo de Modo estudio con estrategia Normal/Aleatoria por usuario+examen. Iniciar la estrategia opuesta marca el anterior como abandonado y crea un nuevo intento.
- Puede existir como máximo una Sesión de falladas activa por usuario y, por separado, como máximo un Modo examen activo por usuario.
- Una respuesta de intento guarda la referencia estable a la pregunta de origen, selección, estado de corrección y marcas temporales necesarias para persistencia/revisión. Para revisión histórica, el intento aporta además la versión exacta del examen desde la que debe cargarse el contenido.
- El progreso por pregunta guarda contadores de respuesta, aciertos, errores, racha consecutiva y estado never_failed/pending/mastered o equivalente y se ancla a la identidad estable de pregunta, no al `exam_version`.
...
Se han confirmado **dos seams principales**, deliberadamente pocos y altos. Ambos comparten el mismo esquema canónico de Banco de exámenes: al menos un fixture utilizado por el Seam 2 debe ser producido por el importador real del Seam 1 y validado contra el mismo schema, para detectar drift entre productor y consumidor sin crear un tercer seam.
...
- reinicio de racha tras un fallo o una blanca de Modo examen;
- conservación de `question_progress` al publicar una nueva versión del mismo examen;
- exclusión de Preguntas anuladas y reservas no utilizadas del pool artificial;
- bloqueo de Examen artificial cuando el pool tiene menos de 75 preguntas activas distintas;
...
- bloqueo local y posterior sincronización si el deadline vence durante un corte de red o suspensión del dispositivo;
- rechazo server-side de cualquier modificación de respuesta posterior al deadline;
...
- comprobación negativa de que la RPC/vista de agregados nunca expone respuestas crudas ni intentos detallados de otro usuario;
...
- rechazo/recuperación segura ante una escritura local obsoleta después de que otro dispositivo haya avanzado secuencialmente el mismo intento;
...
Criterio de salida de este seam: los criterios funcionales de la spec deben poder demostrarse mediante recorridos automatizados o, donde la plataforma no permita una automatización fiable, mediante una lista explícita y mínima de validación manual. La expiración tras suspensión prolongada/offline real del dispositivo puede permanecer como prueba manual dirigida si automatizarla de forma fiable exige manipular el reloj del sistema.
...
- Edición simultánea soportada del mismo intento desde varios dispositivos; el cambio de dispositivo de la v1 es secuencial.
...
- Una corrección posterior de un examen publicado debe crear una nueva versión sin invalidar la revisión de intentos históricos. Las versiones antiguas necesarias para Historial deben seguir desplegadas y direccionables de forma estable.
- El ranking usa siempre `exam_elapsed_ms` del intento de Modo examen. Si el intento expira, su tiempo empleado equivale a la duración oficial; el Tiempo activo de estudio nunca participa en ese desempate.
